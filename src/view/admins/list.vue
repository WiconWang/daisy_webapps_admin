...
// 列表、
import {getAdminList, getAdminInfo, saveAdminStatus, delAdminInfo} from '@/api/admin'
...
export default {
  ...
  methods: {
    ...
    // 取API数据
    fetchTableData () {
      getAdminList(this.pageNum, this.pageSize).then(res => {
    ...
    switchRow (id, status) {
      let _this = this;
       saveAdminStatus(id, status===1?0:1).then(function (data) {
         if (data.data.code === 0) {
    ...
    delRow(id) {
      let _this = this;
      delAdminInfo(id).then(function (data) {
        // console.log(data);
        if (data.data.code === 0) {
